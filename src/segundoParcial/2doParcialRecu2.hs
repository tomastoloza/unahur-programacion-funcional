...
mapMany :: [a -> a] -> [a] -> [a]
mapMany = foldl (\acc f -> map f . acc) id
...
sumaYProd = foldr (\x (rc1,rc2) -> (rc1+x,rc2*x)) (0,1)
...
foldA f g (Nodo x izq med der) = g x (foldA f g izq) (foldA f g med) (foldA f g der)

maximo :: Arbol Int -> Int
maximo = foldA id (\x ri rm rd -> max x (max ri (max rm rd)))